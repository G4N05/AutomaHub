...
local function initTwistOfFate()
    ...
    local losIgnoreCache = {}

    local function aimHasLOS(part)
        if not part or not part.Parent then return false end
        local origin = AimCamera.CFrame.Position
        
        table.clear(losIgnoreCache)
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr.Character then
                table.insert(losIgnoreCache, plr.Character)
            end
        end
        losRaycastParams.FilterDescendantsInstances = losIgnoreCache
    ...
    local aimRenderConn = RunService.RenderStepped:Connect(function()
        ...
        local silentAimEnabled = CONFIG.silentAimGun
        local aimLockEnabled = CONFIG.aimLock
...
local Logic = {
    ...
    ESP = ESP,
    Aim = {
        SetAimGunMode = function(mode: string)
            if mode == "Off" then
                CONFIG.silentAimGun = false
                CONFIG.aimLock = false
            elseif mode == "Aim Lock" then
                CONFIG.silentAimGun = false
                CONFIG.aimLock = true
            elseif mode == "Silent Aim" then
                CONFIG.silentAimGun = true
                CONFIG.aimLock = false
