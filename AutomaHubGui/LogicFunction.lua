...
local function initTwistOfFate()
    ...
    -- ponytail: reuse RaycastParams and pre-build ignore list in RenderStepped to optimize performance
    ...
    local characterIgnoreList = {}
    local function updateIgnoreList()
        table.clear(characterIgnoreList)
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr.Character then
                table.insert(characterIgnoreList, plr.Character)
            end
        end
        losRaycastParams.FilterDescendantsInstances = characterIgnoreList
    end

    -- ponytail: eliminate points table creation to completely remove GC allocation overhead inside the LOS check
    local function aimHasLOS(part)
        ...
        -- Check HRP
        if Workspace:Raycast(origin, part.Position - origin, losRaycastParams) == nil then return true end
        
        -- Check Head
        local head = char:FindFirstChild("Head") :: BasePart?
        if head and Workspace:Raycast(origin, head.Position - origin, losRaycastParams) == nil then return true end
        
        -- Check Top
        if Workspace:Raycast(origin, (part.Position + Vector3.new(0, 2.5, 0)) - origin, losRaycastParams) == nil then return true end
    ...
    local aimRenderConn = RunService.RenderStepped:Connect(function()
        ...
        -- ponytail: update the Raycast ignore list once per frame instead of per target
...
local Logic = {
    ...
    Aim = {
        ...
        SetMultiConfig = function(selected: { [string]: boolean })
            CONFIG.silentAimGun = selected["Silent Aim"] == true
            CONFIG.aimLock = selected["Aim Lock"] == true
            CONFIG.aimWallcheck = selected["Wallcheck"] == true
            CONFIG.aimEnableLead = selected["Lead Prediction"] == true
            CONFIG.aimShowFov = selected["Show FOV"] == true
        end
    }
}
