...
local function initVeil()
            ...
            if plr ~= LocalPlayer then
                ...
                if part then
                    ...
                    if onScreen then
                        ...
                        if dist >= CONFIG.veilMinDist and dist <= CONFIG.veilMaxDist then
                            local d = (Vector2.new(sp.X, sp.Y) - center).Magnitude
    ...
    local function veilSolveBallistic(origin, target, speed, g)
        ...
        local speedCos = speed * cosTheta
        local tof = (speedCos > 0.001) and (dx / speedCos) or 0
        return dir, tof
    end
...
local Logic = {
    ...
    Aim = {
        ...
        SetVeilLeadMult = function(mult: number)
            CONFIG.veilLeadMult = mult
        end,
        SetVeilMinDist = function(dist: number)
            CONFIG.veilMinDist = dist
        end,
        SetVeilMaxDist = function(dist: number)
